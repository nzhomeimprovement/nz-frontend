...
function getHomeAdditionPhotos() {
  const dir = path.join(process.cwd(), "public", "img", "gallery", "home-addition");
  return fs
    .readdirSync(dir)
    .filter((f) => /\.(jpe?g|png|webp)$/i.test(f))
    .sort((a, b) => parseInt(a) - parseInt(b))
    .map((f) => ({ src: `/img/gallery/home-addition/${f}`, alt: "Home addition project - NZ Home Improvement Stamford CT" }));
}

export default function HomeAdditionPage() {
  const additionPhotos = getHomeAdditionPhotos();
  const [introImg1, introImg2] = additionPhotos;
  ...
  return (
    <>
      ...
      {/* ── Portfolio Carousel ── */}
      <PortfolioCarousel heading="Home Addition Projects" photos={additionPhotos} />
        ...
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-12">
            ...
            <p className="text-gray-500 text-sm md:text-base mt-4 max-w-2xl mx-auto leading-relaxed">
              Browse our completed home addition projects across Stamford, CT and Fairfield County.
            </p>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 md:gap-4">
            {additionPhotos.map((photo) => (
